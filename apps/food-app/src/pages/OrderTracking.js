...
const OrderTracking = () => {
  ...
  const fetchTracking = async () => {
    try {
      // Try food orders first, then grocery
      let res = await axios.get(`/api/food/orders/${id}/tracking`).catch(() => null);
      if (!res) {
        res = await axios.get(`/api/grocery/orders/${id}/tracking`).catch(() => null);
      }
      if (res) {
