...
const Cart = () => {
  ...
  const getGrandTotal = () => getSubtotal() + getDeliveryFee();
            ...
            <div key={idx} className="cart-item-card">
              <div className="cart-item-details">
                ...
                <div className="quantity-controls">
                  <button className="quantity-btn" onClick={() => updateQuantity(idx, item.quantity - 1)} aria-label="Decrease quantity">−</button>
                  <span className="quantity-display">{item.quantity}</span>
                  <button className="quantity-btn" onClick={() => updateQuantity(idx, item.quantity + 1)} aria-label="Increase quantity">+</button>
                  <button className="quantity-btn" onClick={() => removeItem(idx)} style={{ marginLeft: 'auto', color: 'var(--danger-color)', borderColor: 'var(--danger-color)' }} aria-label="Remove item">×</button>
                </div>
              </div>
