...
const ContentManagement = () => {
  const [activeModule, setActiveModule] = useState('restaurants'); // Default to restaurants, but can manage all modules
          ...
          <div className="form-group">
            ...
            {formData.image && (
              <div style={{ marginTop: '10px' }}>
                <img src={formData.image} alt="Preview" style={{ maxWidth: '200px', maxHeight: '150px', borderRadius: '8px' }} />
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {activeModule === 'restaurants' && (
              <p className="item-meta">
                Cuisine: {item.cuisine} | Delivery: {item.deliveryTime} mins | Fee: <strong>₹{item.deliveryFee || 0}</strong>
            ...
            {activeModule === 'grocery' && (
              <p className="item-meta">
                Category: {item.category} | Price: <strong>₹{item.price}/{item.unit}</strong> | Stock: <strong>{item.stock}</strong>
                {item.rating > 0 && ` | ⭐ ${item.rating.toFixed(1)}`}
                {(item.discount > 0 || item.discountAmount > 0) && (
                  <span style={{ color: '#28a745', fontWeight: 'bold' }}>
                    {' | '}
                    ...
                    {item.discount > 0 && item.discountAmount > 0 && ' + '}
                    {item.discountAmount > 0 && `₹${item.discountAmount} OFF`}
                    {((item.discount > 0 || item.discountAmount > 0) && item.price) && (
            ...
            {activeModule === 'services' && (
              <p className="item-meta">
                Category: {item.category} | Providers: {item.providers?.length || 0}
                {item.rating > 0 && ` | ⭐ ${item.rating.toFixed(1)}`}
                {(item.discount > 0 || item.discountAmount > 0) && (
                  <span style={{ color: '#28a745', fontWeight: 'bold' }}>
                    {' | '}
